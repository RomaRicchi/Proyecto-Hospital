import { Admision, Paciente, ObraSocial, Usuario, Cama, Movimiento, MovimientoHabitacion } from '../models/index.js';
...
export const buscarAdmisionVigente = async (req, res) => {
  try {
    ...
    if (!paciente) return res.json({ success: false, message: 'Paciente no encontrado' });
...
export const darAltaPaciente = async (req, res) => {
  try {
    ...
    if (!admision) return res.json({ success: false, message: 'No hay admisión activa' });
